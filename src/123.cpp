...
{
  //A_B
  Eigen::Quaterniond q_A_B(std::sqrt(2)/2, 0, 0, std::sqrt(2)/2);
  ...
  Eigen::Quaterniond q_B_C(std::sqrt(3)/2, 0, 0, 1/2);
  Eigen::Vector3d v_B_C(1, 0, 0);
  ...
  //C_A
  Eigen::Quaterniond q_A_C;
  Eigen::Vector3d v_A_C;

  q_A_C =  q_A_B * q_B_C ;
  v_A_C =  q_B_C.inverse() * v_A_B +  v_B_C;
  ...
  //A_A1
  Eigen::Quaterniond q_A_A1(1, 0, 0, 0);
  Eigen::Vector3d v_A_A1(0, 0, 0);

  tf::Transform tf_A_A1;
  tf::Quaternion tf_q_A_A1(q_A_A1.x(),q_A_A1.y(),q_A_A1.z(),q_A_A1.w());
  tf::Vector3 tf_v_A_A1(v_A_A1(0), v_A_A1(1), v_A_A1(2));
